...
describe("VendingMachine", function () {
  ...
  beforeEach(async function () {
    ...
    const VendingMachineContract = await ethers.getContractFactory(
      "VendingMachine"
    );
  ...
  it("should allow the owner to restock the vending machine", async function () {
    ...
    const remaindDonutsAfterPurchase = Number(
      await vendingMachine.getVendingMachineBalance()
    );
    console.log(
    ...
    const remaindDonutsAfterRestock = Number(
      await vendingMachine.getVendingMachineBalance()
    );
    console.log(
      `the machine has after restock 5 donuts: ${remaindDonutsAfterRestock} donuts. `
    );
    expect(remaindDonutsAfterRestock).to.equal(
      remaindDonutsAfterPurchase + RESTOCK_AMOUNT
    );
  });
